...
function countdownCard(c){return '<div class="card delete-hold" data-count="'+c.id+'" title="長押しで編集・削除"><div class="count"><div><b>'+esc(c.title)+'</b><div class="meta">'+fmt(c.date)+'</div></div><div class="days '+(daysLeft(c.date)<0?"danger":daysLeft(c.date)===0?"today":"")+'">'+rem(c.date)+'</div></div></div>'}
function renderHome(){
 refresh();
...
function longPress(el,fn){let timer;el.addEventListener("pointerdown",()=>timer=setTimeout(fn,600));["pointerup","pointercancel","pointerleave"].forEach(x=>el.addEventListener(x,()=>clearTimeout(timer)));el.addEventListener("contextmenu",e=>e.preventDefault())}
